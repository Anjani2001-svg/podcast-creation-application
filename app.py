...
def _resolve_course_folder(service, root_folder_id: str, course_name: str, status_cb=None):
    """Find or create a subfolder named after the course inside root_folder_id.
    Returns (folder_id, display_name)."""
    def _cb(s):
        if status_cb:
            status_cb(s)

    safe_course = course_name.strip() or "Uncategorised"
    # Escape single quotes for the Drive query
    escaped = safe_course.replace("'", "\\'")

    _cb(f"📂 Looking for course folder '{safe_course}'…")
    ...
    try:
        ...
        if folders:
            ...
            _cb(f"   ✅ Found existing folder '{safe_course}'")
            return folder_id, safe_course
    except HttpError:
        pass

    _cb(f"   📁 Creating course folder '{safe_course}'…")
    new_id = _create_subfolder(service, root_folder_id, safe_course)
    if new_id:
        _cb(f"   ✅ Created '{safe_course}'")
        return new_id, safe_course

    _cb(f"   ⚠️ Could not create '{safe_course}' — uploading to root folder")
    return root_folder_id, "root (fallback)"


def _gdrive_upload(data: bytes, filename: str, service, folder_url: str,
                   course_name: str = "", status_cb=None):
    ...
    target_id, folder_label = _resolve_course_folder(
        service, root_id, course_name or "Uncategorised", status_cb=status_cb)
...
if GDRIVE_AVAILABLE:
    with st.expander("☁  Google Drive Connection",
        ...
        if _drive_service and GDRIVE_FOLDER_URL:
            ...
            st.caption(f"📂 Videos are organised into subfolders by course name")
